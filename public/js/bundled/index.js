...
const login = async (email, password)=>{
    try {
        ...
        /** Showing alert and redirecting to home page */ if (response.data.status === 'success') {
...
const logout = async ()=>{
    try {
        ...
        if (response.data.status === 'success') location.reload(true); // 'true' -> reload from the server and not from the browser cache
    } catch (err) {
        (0, _alerts.showAlert)('error', 'Error in logging out');
    }
};

},{"axios":"jo6P5","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./alerts":"6Mcnf"}],"jo6P5":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const showAlert = (type, message)=>{
    hideAlert();
    /** type === 'success' || 'error' */ const markup = `<div class='alert alert--${type}'>${message}</div>`;
...
const displayMap = (locations)=>{
    mapboxgl.accessToken = 'pk.eyJ1Ijoia2Fkd2V5MzAiLCJhIjoiY21kdnlyNjc3MXNndzJscjNjM3dlY2FwdCJ9.9Sftq76ogwOLirdDBbHpXA';
    const map = new mapboxgl.Map({
        container: 'map',
        style: 'mapbox://styles/kadwey30/cmdvytt6p003r01r5458t9n97',
        //   center: [-74.5, 40], // starting position [lng, lat]
        //   zoom: 9, // starting zoom
        //   interactive: false,
        scrollZoom: false
    });
    /** The area that will be displayed on the map */ const bounds = new mapboxgl.LngLatBounds();
    ...
    /** Adding bounds to the map */ map.fitBounds(bounds, {
        padding: {
            top: 200,
            bottom: 170,
            left: 100,
